...
Long-page layout checks cover Comfort, Wide, and Full editor widths beside narrow and wider chat panels. Opening chat and focusing its composer must not scroll the document horizontally or clip the wiki. The fixture also removes the editor shrink rule to prove that it catches the original overflow. Saved Markdown is loaded through the real server renderer and checked for emphasis, lists, code, tables, and expandable evidence; canned HTML alone does not establish formatting coverage.
