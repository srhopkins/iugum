---
description: Cycle editor width (comfort / wide / full) via top-bar button
---

# Editor width

No plug needed - SilverBullet's `--editor-width` CSS variable. This page adds a **3-step cycle** (top-bar button) persisted in `clientStore`.
...
actionButton.define {
  icon = "columns",
  description = "Cycle editor width (comfort -> wide -> full)",
  run = function()
    cycleWidth()
  end
}
...
-- Apply when this extension is first indexed, even after the ready event.
applyWidth(currentWidth())
```
